...
# ==========================================
# CONFIGURATION: EXPERIMENT B (BALANCED)
# ==========================================
EXPERIMENT_NAME = "Experiment_B_Balanced"
...
# BALANCED MODE: Prune if score < 40% of the Best Chunk.
# This aligns with TRAAC's medium bin logic.
PRUNING_RATIO = 0.40        
PROBE_WINDOW_SIZE = 2500 # Safety limit for VRAM

# ==========================================
# 1. SETUP
# ==========================================
print(f"--- Hour 10: {EXPERIMENT_NAME} ---")
...
print(f"Model Loaded. Aggressiveness: {PRUNING_RATIO} (Balanced)")
...
def prune_and_refresh(chunks_metadata, attn_vector, window_start, full_input_ids, tokenizer):
    ...
    for i, meta in enumerate(chunks_metadata):
        # Calculate relative position in the attention window
        c_start, c_end = meta["start"], meta["end"]
        rel_start = c_start - window_start
        rel_end = c_end - window_start
        
        # Only score if the chunk is fully inside the probed window
    ...
    # Need candidates to compare against
    ...
    if victim["score"] < threshold:
        ...
        # Reconstruct Text
        # 1. Prompt (Everything before first tracked chunk)
...
print(f"\n\n{'='*40}")
print(f"EXPERIMENT B (BALANCED) RESULTS")
print(f"{'='*40}")
print(f"Accuracy:        {accuracy:.1f}%")
print(f"Avg Reduction:   {avg_reduction:.1f}%")
print(f"{'='*40}")

with open("results_exp_b_balanced.json", "w") as f:
    json.dump(results, f, indent=4)
print(f"Detailed logs saved to results_exp_b_balanced.json")
...
# ==========================================
# CONFIGURATION: EXPERIMENT C (AGGRESSIVE)
# ==========================================
EXPERIMENT_NAME = "Experiment_C_Aggressive"
...
# AGGRESSIVE MODE: Prune anything score < 60% of the Best Chunk.
# This forces the model to keep ONLY the most critical steps.
PRUNING_RATIO = 0.60        
PROBE_WINDOW_SIZE = 2500 # Keep stable window to prevent OOM

# ==========================================
# 1. SETUP
# ==========================================
print(f"--- Hour 11: {EXPERIMENT_NAME} ---")
...
print(f"Model Loaded. Aggressiveness: {PRUNING_RATIO} (High)")
...
def prune_and_refresh(chunks_metadata, attn_vector, window_start, full_input_ids, tokenizer):
    ...
    if victim["score"] < threshold:
        ...
        for meta in chunks_metadata:
            if meta["start"] == victim_start: continue
...
print(f"\n\n{'='*40}")
print(f"EXPERIMENT C (AGGRESSIVE) RESULTS")
print(f"{'='*40}")
print(f"Accuracy:        {accuracy:.1f}%")
print(f"Avg Reduction:   {avg_reduction:.1f}%")
print(f"{'='*40}")

with open("results_exp_c_aggressive.json", "w") as f:
    json.dump(results, f, indent=4)
print(f"Detailed logs saved to results_exp_c_aggressive.json")